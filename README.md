...
### 完全オフライン（サーバ不要・ネットワーク/プロキシ制限に強い）

`build-standalone.cmd` をダブルクリックすると、すべて（Mermaid 含む）を 1 枚に
インライン化した自己完結ファイル **`dist\SeqShelf.html`** を生成して開きます。
このファイルは `file://` でダブルクリックするだけで動作し、**一切ネットワーク通信を行いません**。
社内プロキシやオフライン環境でもそのまま使え、USB で配布しても単体で動きます。

```powershell
npm run build:standalone   # dist/SeqShelf.html を生成
```
...
- **保存・入出力**:
  ...
  - **📦 全データ / 📦 復元** で、編集中の本棚と保存ライブラリを 1 ファイルにまとめて
    書き出し・読込（アプリ更新や別 PC への配布時の引き継ぎ用）
  - **Mermaid** / **SVG** で表示中の図を書き出し（他ツール貼付・画像化）
- **完全オフライン動作**: `build-standalone.cmd` で生成する `dist/SeqShelf.html` は
  Mermaid を含め全部入りの 1 ファイル。`file://` で開くだけで動き、ネットワーク通信は一切なし。
...
## データの保存場所とアプリ更新時の引き継ぎ

本棚のデータは **ブラウザの localStorage** に保存されます（外部サーバには一切送信されません）。
使われるキーは以下の 2 つです。

| キー | 内容 |
| --- | --- |
| `seqshelf.shelf.v2`  | いま編集中の本棚（自動保存） |
| `seqshelf.library.v1`| 「💾 本棚保存」で名前を付けて保存したスナップショット集 |

localStorage は **オリジン (URL) ごと** に分離されます。

- 開発サーバ: `http://localhost:5173/`
- スタンドアロン: `file:///<このフォルダ>/dist/SeqShelf.html`

### アプリを更新したとき

- **同じ URL / 同じファイルパスで開く限り**、コードを修正・再ビルドしてもデータは
  自動で引き継がれます（localStorage はソース変更では消えません）。
- 次の操作をするとデータが失われます。
  - `dist/SeqShelf.html` を別フォルダに移動する
  - 別 PC・別ブラウザに配布する
  - ブラウザの「サイトデータ削除」を行う

### 引き継ぎ手順（推奨）

1. 旧環境のツールバー右側にある **「📦 全データ」** を押し、JSON を保存します。
   （編集中の本棚 + 保存ライブラリ全部が 1 ファイルになります。）
2. 新環境でアプリを開き、空のままで表示される **黄色いバナー**、または
   ツールバーの **「📦 復元」** から、その JSON を読み込みます。
3. 編集中の本棚と保存ライブラリの両方が復元されます。

保存場所は、ツールバーの **「ℹ 保存先」** からいつでも確認できます。
...
| `npm run build:standalone` | 型チェック + ビルド + 単一HTML生成 (`dist/SeqShelf.html`、オフライン用) |
